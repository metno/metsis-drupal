 ...
 */
class MetsisQSearchResultsForm extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'metsis_qsearch_results_form';
  }
  /**
   * {@inheritdoc}
   */
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form ['table'] = $this->qsearch_get_ctable($form_state, (int) $params['page']);

    /**
     * @FIXME
     * This validation executes when going back to search. Should only be invoked when adding to basket_view
     * Commented out for now
     */
    //$form['#validate'][] = 'adc_min_select_validate';
    //var_dump($_SESSION['qsearch']['number_of_pages']);

    /**
     * @FIXME $_SESSION should be changed to private tempstore
     */
    ...
    $form['adc_buttons_wrapper']['pagination'] = [
      '#markup' => $pager_markup,
    ...
    if (BASKET_ELEMENTS_VISIBLE) {
      if (count($form['table']['table']['#options']) > 0) {
        $form['adc_buttons_wrapper']['add_to_basket'] = array(
          ...
          '#validate' => array('adc_min_select_validate'),
          '#attributes' => array(
            'class' => array(
              'adc-button-small',
            ),
    ...
    $form['#attached']['library'][] = 'metsis_lib/utils';
    $form['#attached']['library'][] = 'metsis_lib/popups';
    $form['#attached']['library'][] = 'metsis_qsearch/qsearch.misc';
    $form['#attached']['library'][] = 'metsis_qsearch/qstyles';
    $form['#attached']['library'][] = 'metsis_lib/map_thumbnails';
    $form['#attached']['library'][] = 'metsis_qsearch/pagination';

    return $form;
    }

public function q_results_tabular_form_submit($form, &$form_state) {
...
public function qsearch_get_ctable($form_state, $page_number) {
    ...
    if (!empty($solr_obj['error'])) {
      ...
      //\Drupal::messenger()->addStatus(t("Number of datasets found") . ": " . $solr_obj['response']['numFound']);
      $this->messenger()->addStatus("Number of datasets found" . ": " . $solr_obj['response']['numFound']);
    }
    ...
    foreach ($solr_obj['response']['docs'] as $doc) {
      ...
      //  $target_url = $base_url . '/' . 'metsis/map/wms?dataset=' . $doc[METADATA_PREFIX . 'metadata_identifier'] . '&solr_core=' . SOLR_CORE_PARENT;
        $url = Url::fromRoute('metsis_qsearch.wms', [
          'dataset' => $doc[METADATA_PREFIX . 'metadata_identifier'],
          'solr_core' => SOLR_CORE_PARENT,
        ], ['absolute' => TRUE]);
      ...
      if (count($tmp_thumb_response['response']['docs']) > 0) {
        if (isset($tmp_thumb_response['response']['docs'][0]['thumbnail_data'])) {
          if ($tmp_thumb_response['response']['docs'][0]['thumbnail_data'] != "") {
            ...
            $thumbnail_data = PublicStream::basePath() . "/" . "missing_map_place_holder.png";
      ...
      if ($metsis_conf['results_cloud_cover_value_visible'] === TRUE) {
        ...
        \Drupal::logger('metsis-qsearch')->debug('Kewords list string is: ' . $keywords);
      } if (RESULTS_ABSTRACT_VISIBLE) {
      ...
      if (adc_has_feature_type($doc[METADATA_PREFIX . 'metadata_identifier'], "timeSeries") === 1) {
        ...
        //$visualize_url = "/" . 'tsx' . "?metadata_identifier=" . $doc[METADATA_PREFIX . 'metadata_identifier'];
        ...
        $url = Url::fromRoute('metsis_csv.csvform', [
          'metadata_identifier' => $doc[METADATA_PREFIX . 'metadata_identifier'],
          'calling_results_page' => $page_number,
        ], ['absolute' => TRUE]);
        $ascii_url = $url->toString();
        //$ascii_url = "/" . 'csx' . "?metadata_identifier=" . $doc[METADATA_PREFIX . 'metadata_identifier'];
    ...
    // l() expects a Url object, created from a route name or external URI.
     $options[$dataset_name] = array('datasetName' => $this->l($this->t($dataset_name), $data_access_HTTP, array('external' => TRUE, 'html' => TRUE, 'attributes' => array('class' => array('ext_data_source'),))), 'institutions' => '', 'title' => '', 'project' => '', 'keywords' => '', 'abstract' => '', 'collection_period' => t($collection_period), '#attributes' => array('class' => array('childdataset',)));
